...
snake_blocksize = 10
snake_speed = 25
high_scores_list = [0]*5

def snake_body(snake_blocksize, snake_list): # Draw the snake body
    for block in snake_list:
        pygame.draw.rect(dis, green,[block[0], block[1], snake_blocksize, snake_blocksize]) # coordinates, rect size

def score_counter(score_count):  #Displays current score
    value = score_font.render("Your Score: " + str(score_count), True, white)
    dis.blit(value, [10,0])

def message(msg, colour): #Displays Game Over message
    mesg = font_style.render(msg, True, colour)
    dis.blit(mesg,[40, 50])

def high_score(high_scores_list, colour): # Displays top 5 highest scores
...
def gameloop(): #Run the game
    ...
    while not game_close:
        ...
        while game_over == True: # When player loses
            while run_once == 0:
                high_scores_list.append(score_count)
                high_scores_list.sort(reverse=True)
                high_scores_list.pop()
                run_once += 1
            message("GAME OVER! Press R to play again.", yellow)
            high_score(high_scores_list, cyan)
